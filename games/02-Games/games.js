 ...
 /* ========================================================= 
    ...
    const CONFIG = { 
        ...
        FAVORITES_KEY: "sfvx_favorites", 
         PLAYS_KEY: "sfvx_play_counts",      
        RATINGS_KEY: "sfvx_game_ratings",  
...
/* ========================================================= 
    ...
    const TRANSLATIONS = { 
        ...
        es: { 
            ...
            heroDescription: "Descubre una nueva colección de juegos y prepárate para tu próxima experiencia.", 
            playNow: "Jugar Ahora", 
            details: "Detalles", 
        ...
        fr: { 
            ...
            heroDescription: "Découvrez une nouvelle collection de jeux et préparez-vous pour votre prochaine expérience.", 
        ...
        pt: { 
            ...
            heroDescription: "Descubra uma nova coleção de jogos e prepare-se para sua próxima experiência.", 
            playNow: "Jogar Agora", 
            details: "Detalhes", 
...
/* ========================================================= 
    ...
    function fuzzySearch(query, games) { 
        ...
        return games.map(game => { 
            ...
            terms.forEach(term => { 
                const titleScore = getSimilarityScore(term, title); 
                const categoryScore = getSimilarityScore(term, category); 
                const tagsScore = getSimilarityScore(term, tags); 
                const descriptionScore = getSimilarityScore(term, description); 

                const score = Math.max(titleScore, categoryScore, tagsScore, descriptionScore); 
                totalScore = Math.max(totalScore, score); 
            }); 

            return { ...game, searchScore: totalScore }; 
        }).filter(game => game.searchScore > 0.2); 
    }
   /* ========================================================= 
    7. DATA LOADING 
    ========================================================= */ 
    ...
    /* ========================================================= 
    7.5 USER INTERACTION (Plays & Ratings) 
    ========================================================= */ 

    function getGamePlays(id) {
        try {
            const counts = JSON.parse(localStorage.getItem(CONFIG.PLAYS_KEY)) || {};
    ...
    function incrementGamePlays(id) {
        try {
            let counts = JSON.parse(localStorage.getItem(CONFIG.PLAYS_KEY)) || {};
            counts[id] = (Number(counts[id]) || 0) + 1;
            localStorage.setItem(CONFIG.PLAYS_KEY, JSON.stringify(counts));
        } catch (e) {
            console.warn("Could not update plays:", e);
        }
    }

    function getGameRating(id) {
        try {
            const ratings = JSON.parse(localStorage.getItem(CONFIG.RATINGS_KEY)) || {};
            return Number(ratings[id]) || 5.0; // افتراضي لو مافيش تقييم
        } catch (e) {
            return 5.0;
        }
    }
    ...
    function rateGame(id, score) {
        try {
            let ratings = JSON.parse(localStorage.getItem(CONFIG.RATINGS_KEY)) || {};
            ratings[id] = Number(score);
            localStorage.setItem(CONFIG.RATINGS_KEY, JSON.stringify(ratings));
            applyFilters(); // إعادة ترتيب الألعاب فوراً حسب التقييم الجديد
        } catch (e) {
            console.warn("Could économiques save rating:", e);
        }
    }

/* ========================================================= 
    8. GAME CARD (Interactive & Clean Design) 
    ========================================================= */ 
    function renderGameCard(game, showFavorite = true) { 
        ...
        // جلب عدد الزيارات والتقييم المخزن محلياً لهذه اللعبة بالذات
        const currentPlays = getGamePlays(id);
        const currentRating = getGameRating(id);

        const tags = game.tags 
            ? String(game.tags).split(",").slice(0, 2).map(tag => tag.trim()).filter(Boolean).join(", ") 
            : ""; 
        ...
        return ` 
            <article class="game-card" data-id="${escapeHTML(id)}" data-category="${escapeHTML(category)}" tabindex="0" role="button" aria-label="${escapeHTML(title)}"> 
                ${favoriteButton} 
                <div class="game-card-image" style="background: radial-gradient(circle at center, rgba(124,60,255,.15), transparent 45%), var(--bg-elevated);"> 
                    ${imageHTML} 
                ...
                <div class="game-card-content"> 
                    ...
                    <!-- شريط المعلومات السفلي (زر اللعب + عدد الزيارات بتصميم شيك) -->
                    <div style="display: flex; align-items: center; justify-content: space-between; margin-top: 10px; padding-top: 8px; border-top: 1px solid rgba(255,255,255,0.08);">
                        ...
                        <div style="display: flex; align-items: center; gap: 4px; font-size: 12px; color: var(--text-muted, #aaa);">
                            <span>🔥</span>
                            <span class="play-count-view-${escapeHTML(id)}">${currentPlays} لعب</span>
                        </div>
                    </div>

                    <!-- نظام النجوم التفاعلي الحقيقي (قابل للضغط لاختيار التقييم) -->
                    <div class="game-rating-stars-container" data-game-id="${escapeHTML(id)}" style="display: flex; align-items: center; justify-content: center; gap: 2px; margin-top: 10px; background: rgba(0,0,0,0.2); padding: 5px; border-radius: 8px;">
                        <div class="stars-list" style="display: flex; gap: 2px; cursor: pointer;">
                            ${[1, 2, 3, 4, 5].map(star => `
                                <span onclick="handleStarClick('${escapeHTML(id)}', ${star}, this)" style="font-size: 16px; color: ${star <= Math.round(currentRating) ? '#ffc107' : '#555'}; transition: transform 0.1s;" onmouseover="this.style.transform='scale(1.2)'" onmouseout="this.style.transform='scale(1)'">★</span>
                            `).join('')}
                        </div>
                        <span class="rating-number-${escapeHTML(id)}" style="font-size: 11px; margin-right: 6px; color: #ffc107; font-weight: bold;">(${currentRating.toFixed(1)})</span>
                    </div>
                </div> 
            </article> 
        `; 
    ...
    function bindGameCardEvents(grid) { 
        ...
        grid.querySelectorAll(".game-card").forEach(card => { 
            card.addEventListener("click", event => { 
                if (event.target.closest(".game-card-favorite") || event.target.closest(".game-card-play")) { 
                    return; 
                } 
...
/* ========================================================= 
    ...
    function renderPopularGames() { 
        const popular = [...state.allGames].sort((a, b) => { 
            // جلب عدد مرات اللعب والتقييم الحقيقي المخزن محلياً للمستخدمين
            const aPlays = getGamePlays(a.id) || Number(a.popularity ?? a.views ?? 0);
            const bPlays = getGamePlays(b.id) || Number(b.popularity ?? b.views ?? 0);
            
            const aRate = getGameRating(a.id);
            const bRate = getGameRating(b.id);

            // الترتيب المشترك: دمج عدد مرات اللعب الفعلي مع التقييم ليكون الترتيب ذكياً وحقيقياً
            return (bPlays + bRate * 10) - (aPlays + aRate * 10); 
    ...
    function renderFilteredGames() { 
        ...
        if (DOM.searchEmpty) { 
            DOM.searchEmpty.hidden = !(state.isSearching && state.filteredGames.length === 0); 
        } 
    }
...
/* ========================================================= 
    ...
    function renderCategories() { 
        ...
        DOM.categoriesList.innerHTML = categories.map(category => { 
            return ` 
                <button class="category-button ${category === state.currentCategory ? "active" : ""}" data-category="${category}" type="button"> 
                    ${escapeHTML(translate(category))} 
                </button> 
            `; 
        }).join(""); 
    ...
    function applyFilters() { 
        ...
        if (state.currentCategory !== "all") { 
            games = games.filter(game => 
                String(game.category || "").toLowerCase() === state.currentCategory.toLowerCase() 
            ); 
        } 
        ...
        if (!state.isSearching && state.currentCategory === "all") { 
            renderPopularGames(); 
            renderNewGames(); 
        } else { 
            const newSection = DOM.newGamesSection; 
            if (newSection) { 
...
function playGame(id) { 
        ...
        // زيادة عداد الزيارات فوراً عند الضغط على اللعبة
        incrementGamePlays(id);

        const url = `${CONFIG.GAME_PLAYER_URL}?id=${encodeURIComponent(id)}`; 
...
/* ========================================================= 
    ...
    function toggleFavorite(id) { 
        ...
        const index = state.favorites.findIndex(item => normalizeFavoriteId(item) === favoriteId); 
    ...
    function loadFavorites() { 
        try { 
            ...
            state.favorites = Array.isArray(parsed) ? parsed.map(item => String(item)) : []; 
        } catch (error) { 
    ...
    function showFavorites() { 
        const favorites = state.allGames.filter(game => 
            state.favorites.includes(String(game.id)) 
        ); 

        state.filteredGames = favorites; 
        state.isSearching = true; 
        state.searchQuery = ""; 
        ...
        renderCategories(); 
        renderGames(DOM.popularGrid, favorites); 

        if (DOM.newGamesSection) { 
            DOM.newGamesSection.style.display = "none"; 
        } 

        if (DOM.searchEmpty) { 
            DOM.searchEmpty.hidden = favorites.length > 0; 
        } 
    }
 
/* ========================================================= 
   16. STATS & LANGUAGE & THEME & UI ENGINE
   ========================================================= */ 

function updateStats() {
    if (DOM.totalGamesCount) {
        DOM.totalGamesCount.textContent = state.allGames.length;
    }
    ...
    if (DOM.newGamesCount) {
        DOM.newGamesCount.textContent = Math.min(CONFIG.NEW_COUNT, state.allGames.length);
    }
}
...
function applyTranslations() {
    ...
    document.querySelectorAll("[data-i18n-aria]").forEach(element => {
        ...
        if (language[key] !== undefined) {
            element.setAttribute("aria-label", language[key]);
        }
    });
    ...
    if (DOM.languageOptions) {
        DOM.languageOptions.forEach(option => {
            option.classList.toggle("active", option.dataset.language === state.currentLanguage);
        });
    }
...
function toggleDropdownState(isOpen) {
    if (!DOM.languageDropdown) return;
    DOM.languageDropdown.hidden = !isOpen;
    DOM.languageButton?.setAttribute("aria-expanded", String(isOpen));
    document.getElementById("languageSelector")?.classList.toggle("open", isOpen);
}

const openLanguageDropdown = () => toggleDropdownState(true);
const closeLanguageDropdown = () => toggleDropdownState(false);

function toggleLanguageDropdown() {
    if (!DOM.languageDropdown) return;
    toggleDropdownState(DOM.languageDropdown.hidden);
}
...
function updateSidebar() {
    ...
    DOM.menuButton?.setAttribute("aria-expanded", String(state.isSidebarOpen));
    document.body.style.overflow = state.isSidebarOpen ? "hidden" : "";
}

const openSidebar = () => { state.isSidebarOpen = true; updateSidebar(); };
const closeSidebar = () => { state.isSidebarOpen = false; updateSidebar(); };
const toggleSidebar = () => { state.isSidebarOpen = !state.isSidebarOpen; updateSidebar(); };
...
function setupNavigation() {
    document.querySelectorAll(".sidebar-link").forEach(link => {
        link.addEventListener("click", event => {
            ...
            const actions = {
                home: () => { resetView(); scrollToTop(); },
                games: () => { resetView(); scrollToTop(); },
                popular: showPopularSection,
                new: showNewSection,
                recent: showRecentGames,
                favorites: showFavorites,
                random: playRandomGame,
                leaderboard: () => showComingSoon(section),
                achievements: () => showComingSoon(section)
            };
...
function showNewSection() {
    ...
    state.isSearching = false;
    renderNewGames();
    document.getElementById("newGamesSection")?.scrollIntoView({ behavior: "smooth", block: "start" });
}

function showRecentGames() {
    resetSectionState();
    state.isSearching = true;
    const recent = [...state.allGames].reverse().slice(0, CONFIG.GAMES_PER_PAGE);
    state.filteredGames = recent;
    renderGames(DOM.popularGrid, recent);
    if (DOM.newGamesSection) DOM.newGamesSection.style.display = "none";
    scrollToTop();
}

function showComingSoon(section) {
    if (!DOM.popularGrid) return;
    const title = translate(section);
    DOM.popularGrid.innerHTML = `
        <div class="search-empty" style="grid-column:1/-1;">
            <div class="empty-icon">🏆</div>
            <h2>${escapeHTML(title)}</h2>
            <p>SFV-X ${escapeHTML(title)} will be available soon.</p>
        </div>
    `;
    if (DOM.newGamesSection) DOM.newGamesSection.style.display = "none";
    scrollToTop();
...
function setupEventListeners() {
    ...
    if (DOM.searchInput) {
        DOM.searchInput.addEventListener("input", event => handleSearch(event.target.value));
    ...
    DOM.resetSearchBtn?.addEventListener("click", () => { resetView(); scrollToTop(); });
    DOM.languageButton?.addEventListener("click", event => { event.stopPropagation(); toggleLanguageDropdown(); });
    ...
    DOM.heroInfoButton?.addEventListener("click", () => {
        if (!getFeaturedGame()) return;
        document.getElementById("popularSection")?.scrollIntoView({ behavior: "smooth", block: "start" });
    });

    DOM.heroRandomButton?.addEventListener("click", playRandomGame);
    DOM.randomGameLink?.addEventListener("click", event => { event.preventDefault(); playRandomGame(); });
    DOM.profileButton?.addEventListener("click", () => console.log("SFV-X Profile clicked"));

    document.querySelectorAll(".view-all-button").forEach(button => {
        button.addEventListener("click", () => {
            const view = button.dataset.view;
            if (view === "popular") showPopularSection();
            else if (view === "new") showNewSection();
        });
    });

    window.addEventListener("resize", () => {
        if (window.innerWidth > 850 && state.isSidebarOpen) {
            closeSidebar();
        }
    });
}
...
// دالة تفاعل النجوم الحقيقية بالضغط
function handleStarClick(gameId, score, element) {
    // حفظ التقييم محلياً في الـ localStorage
    rateGame(gameId, score);
    
    // تحديث شكل النجوم بصرياً في نفس البطاقة فوراً وبشكل جذاب
    const starsList = element.parentElement;
    const stars = starsList.querySelectorAll('span');
    
    stars.forEach((s, index) => {
        const starValue = index + 1;
        if (starValue <= score) {
            s.style.color = '#ffc107'; // أصفر ذهبي للنقاط المختارة
        } else {
            s.style.color = '#555';   // رمادي للنقاط غير المختارة
        }
    });

    // تحديث رقم التقييم المجاور مباشرة
    const container = starsList.closest('.game-rating-stars-container');
    const ratingNum = container.querySelector(`[class^="rating-number-"]`);
    if (ratingNum) {
        ratingNum.textContent = `(${score.toFixed(1)})`;
    }
}
async function init() {
    loadTheme();
    ...
    console.log("======================================");
    console.log("SFV-X GAMES ENGINE v2.0.1");
    console.log(`Games: ${state.allGames.length}`);
